...
namespace detail {
  ...
  Paganin::Paganin(settings parameters, acquisition::geometry geom, float* data)
  {
    ...
    fft2d_plan_ = fftwf_plan_dft_r2c_2d(geom.cols, geom.rows, data,
        reinterpret_cast<fftwf_complex*>(&proj_freq_buffer_[0][0]),
        FFTW_ESTIMATE);
    ffti2d_plan_ = fftwf_plan_dft_c2r_2d(geom.cols, geom.rows,
        reinterpret_cast<fftwf_complex*>(&proj_freq_buffer_[0][0]),
        data, FFTW_ESTIMATE);
    paganin_filter_ = util::filter::paganin(geom.rows, geom.cols, parameters.paganin.pixel_size,
  ...
  Filterer::Filterer(settings parameters, acquisition::geometry geom, float* data)
  {
    ...
    fft_plan_ = fftwf_plan_dft_r2c_1d(geom.cols, data,
        reinterpret_cast<fftwf_complex*>(&freq_buffer_[0][0]),
        FFTW_ESTIMATE);
    ffti_plan_ = fftwf_plan_dft_c2r_1d(geom.cols,
        reinterpret_cast<fftwf_complex*>(&freq_buffer_[0][0]),
        data, FFTW_ESTIMATE);
...
void ProjectionProcessor::process(float* data, int proj_id_begin, int proj_id_end)
{
  ...
  env_.spawn(param_.filter_cores, [&](auto& world) {
    ...
    for (auto proj_idx = s; proj_idx < proj_count; proj_idx += p) {
      auto proj = detail::Projection { &data[proj_idx * pixels], geom_.rows, geom_.cols };
      if (flatfielder) {
