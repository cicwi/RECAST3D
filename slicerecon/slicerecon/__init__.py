...
def convert_acquisition_geometry(proj_geom, vol_geom):
    ...
    if geom_type not in ['cone', 'cone_vec', 'parallel3d', 'parallel3d_vec']:
        raise Exception(
            "Only 3D cone-beam or parallel-beam geometries supported")
    ...
    volume_min_point = [
        opts['WindowMinX'], opts['WindowMinY'], opts['WindowMinZ']
    ]
    volume_max_point = [
        opts['WindowMaxX'], opts['WindowMaxY'], opts['WindowMaxZ']
    ]

    return acquisition_geometry(rows=rows,
                                cols=cols,
                                proj_count=proj_count,
                                angles=angles,
                                parallel=parallel,
                                vec_geometry=vec_geometry,
                                volume_min_point=volume_min_point,
                                volume_max_point=volume_max_point)


def get_solver(proj_geom,
               vol_geom,
               slice_size,
               proj_data,
    ...
    for idx in range(proj_data.shape[0]):
        push_projection(solver, idx,
                        np.ascontiguousarray(proj_data[idx, :, :]))
