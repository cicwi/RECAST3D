...
class listener {
  ...
  virtual void register_parameter(
      ...
      std::variant<float, std::vector<std::string>, bool> value)
      = 0;
...
class reconstructor {
  ...
  {
    if (!projection_processor_ || !projection_processor_->filterer) {
      slicerecon::util::log << LOG_FILE << slicerecon::util::lvl::info << "Setting filter before processor is set up, ignoring and continuing." << slicerecon::util::end_log;
    }
    projection_processor_->filterer->set_filter(filter);
  }
